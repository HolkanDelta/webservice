...
export default function ServiceEdit({ servicio }: { servicio: ServiceType }) {
    const taskEditForm = useRef<HTMLFormElement>(null);
    const { data, setData, put, processing, errors } = useForm<ServiceType>({
        id: servicio.id,
        name: servicio.name,
    ...
    const ServiceEditTask: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('servicios.update', { servicio: data.id }), {
            onError: () => {
                if (taskEditForm.current) {
                    const firstErrorElement = taskEditForm.current.querySelector<HTMLElement>('.text-red-600');
    ...
    return (
        <AppLayout>
            <Head title={`Servicios - Editar ${servicio.name}`} />
            <div className="flex h-full flex-1 flex-col gap-6 p-6 max-w-4xl mx-auto w-full">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Editar Servicio</h1>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">Modifica los detalles del servicio {servicio.name}</p>
                    </div>
                </div>

                {/* Form Card */}
                <div className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950 p-6">
                    <form ref={taskEditForm} onSubmit={ServiceEditTask} className="flex flex-col gap-6">
                        ...
                        <div className="flex items-center gap-2 text-zinc-800 dark:text-zinc-200 border-b border-zinc-100 pb-3 dark:border-zinc-900 mb-2">
                            ...
                            <span className="font-bold text-base">Detalles del Servicio</span>
                        </div>
